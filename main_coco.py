...
def main(args):
    ...
    if args.eval:
        ...
        # for i in range(args.dec_layers-1):
            # print('*'*10, f'evaluating decoder layer No.{i}', '*'*10)
            # test_stats, coco_evaluator = evaluate_refinements_specific_layer(model, criterion, postprocessors,
            #                                       data_loader_val, base_ds, device, args.output_dir, refine_stage=0, output_layer=i)
        print('*'*10, f'evaluating decoder original output', '*'*10)
        test_stats, coco_evaluator = evaluate_refinements(model, criterion, postprocessors,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        lr_scheduler.step()
        for i in range(args.num_refines+1):
            test_stats, coco_evaluator = evaluate_refinements(
                model, criterion, postprocessors, data_loader_val, base_ds, device, args.output_dir, refine_stage=i
            )
            if i == 0:
        ...
        if args.output_dir:
            ...
            if save_flag:
                ...
                save_flag = False
            if (epoch + 1) % args.lr_drop == 0 or (epoch + 1) % 10 == 0 or epoch <= 10:
