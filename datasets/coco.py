...
def make_coco_transforms(image_set, max_size=512):
...
def build(image_set, args):
    ...
    mode = 'instances'
    PATHS = {
        "train": (root / "train2014", root / "annotations" / "instances_train2014.json"),
        "val": (root / "val2014", root / "annotations" / "instances_val2014.json"),
    }

    img_folder, ann_file = PATHS[image_set]
    dataset = CocoDetection(img_folder, ann_file, transforms=make_coco_transforms(image_set), return_masks=False, dataset='coco')
    if image_set == "train":
