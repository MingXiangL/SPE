...
class CocoDetection(torchvision.datasets.CocoDetection):
    def __init__(self, img_folder, ann_file, transforms, return_masks, dataset='coco'):
        super(CocoDetection, self).__init__(img_folder, ann_file)
        self._transforms = transforms
...
def build(image_set, args):
    ...
    # else:
    if args.fixed_size:
        dataset = CocoDetection(img_folder, ann_file, transforms=make_coco_transforms_specific_size_fixed(image_set, max_size=args.max_size), return_masks=False, dataset='coco')
    else:
        dataset = CocoDetection(img_folder, ann_file, transforms=make_coco_transforms_specific_size(image_set, max_size=args.max_size), return_masks=False, dataset='coco')
    if image_set == "train":
