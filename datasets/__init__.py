...
def build_dataset(image_set, args):
    ...
    if 'voc' in args.dataset_file:
        args.num_classes = 20

        if args.dataset_file == 'voc':
            return build_voc(image_set, args)

        elif args.dataset_file == 'voc12':
            if image_set == 'val':
                return build_voc(image_set, args)
            else:
        ...
        elif args.dataset_file == 'voc07':
            if image_set == 'val':
                return build_voc(image_set, args)
            else:
                dataset1 = build_voc(image_set, args, anno="pascal_train2007.json")
                dataset2 = build_voc(image_set, args, anno="pascal_val2007.json")
                return torch.utils.data.ConcatDataset([dataset1, dataset2])

        elif args.dataset_file =='voc_voc':
